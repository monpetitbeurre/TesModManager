...
namespace Microsoft.Samples.DirectX.UtilityToolkit
{
    ...
    public class ResourceCache
    {
        ...
        {
            return CreateTextureFromFileEx(device, filename, Constants.Default, Constants.Default, Constants.Default, Usage.None,
                Format.Unknown, Pool.Managed, (Filter)Constants.Default, (Filter)Constants.Default, 0);
        }
        /// <summary>Create a texture from a file</summary>
        public Texture CreateTextureFromFileEx(Device device, string filename, int w, int h, int mip, Usage usage, Format fmt, Pool pool, Filter filter, Filter mipfilter, int colorkey)
        {
            ...
            Texture t = new Texture(device, filename, w, h, mip, usage, fmt, pool, filter, mipfilter, colorkey, false, null);
        ...
        {
            return CreateCubeTextureFromFileEx(device, filename, Constants.Default, Constants.Default, Usage.None,
                Format.Unknown, Pool.Managed, (Filter)Constants.Default, (Filter)Constants.Default, 0);
        }
        /// <summary>Create a cube texture from a file</summary>
        public CubeTexture CreateCubeTextureFromFileEx(Device device, string filename, int size, int mip, Usage usage, Format fmt, Pool pool, Filter filter, Filter mipfilter, int colorkey)
        {
            ...
            // No matching entry, load the resource and add it to the cache
            CubeTexture t = new CubeTexture(device, filename, size, mip, usage, fmt, pool, filter, mipfilter, colorkey, false, null);
            CachedTexture entry = new CachedTexture();
            entry.Source = filename;
            entry.Width = size;
            entry.MipLevels = mip;
        ...
        {
            return CreateVolumeTextureFromFileEx(device, filename, Constants.Default, Constants.Default, Constants.Default, Constants.Default, Usage.None,
                Format.Unknown, Pool.Managed, (Filter)Constants.Default, (Filter)Constants.Default, 0);
        }
        /// <summary>Create a volume texture from a file</summary>
        public VolumeTexture CreateVolumeTextureFromFileEx(Device device, string filename, int w, int h, int d, int mip, Usage usage, Format fmt, Pool pool, Filter filter, Filter mipfilter, int colorkey)
        {
            ...
            // No matching entry, load the resource and add it to the cache
            VolumeTexture t = new VolumeTexture(device, filename, w, h, d, mip, usage, fmt, pool, filter, mipfilter, colorkey, false, null);
            CachedTexture entry = new CachedTexture();
            entry.Source = filename;
            entry.Width = w;
            entry.Height = h;
    ...
    public class ArcBall
    {
        ...
        public static Quaternion QuaternionFromBallPoints(Vector3 from, Vector3 to)
        {
            ...
            Vector3 part = Vector3.CrossProduct(from, to);
    ...
    public abstract class Camera
    {
        ...
        public unsafe virtual void SetViewParameters(Vector3 eyePt, Vector3 lookAtPt)
        {
            ...
            viewMatrix = Matrix.LookAtLeftHanded(eye, lookAt, UpDirection);
        ...
        public virtual void SetProjectionParameters(float fov, float aspect, float near, float far)
        {
            ...
            projMatrix = Matrix.PerspectiveFieldOfViewLeftHanded(fov, aspect, near, far);
        }
    ...
    public class FirstPersonCamera : Camera
    {
        ...
        public override void FrameMove(float elapsedTime)
        {
            ...
            viewMatrix = Matrix.LookAtLeftHanded(eye, lookAt, worldUp);
    ...
    public class ModelViewerCamera : Camera
    {
        ...
        public override void SetViewParameters(Vector3 eyePt, Vector3 lookAtPt)
        {
            ...
            Matrix rotation = Matrix.LookAtLeftHanded(eyePt, lookAtPt, UpDirection);
    ...
    public struct TextHelper
    {
        ...
        /// Draw a line of text
        /// </summary>
        public void DrawStringLine(string text)
        {
            if (textFont == null)
            {
            ...
            textFont.DrawString(textSprite, text, rect, DrawStringFormat.NoClip, color);
        ...
        /// Draw a line of text
        /// </summary>
        public void DrawStringLine(string text, params object[] args)
        {
            // Simply format the string and pass it on
            DrawStringLine(string.Format(text, args));
        }
    ...
    public class Utility
    {
        ...
        public static string FindMediaFile(string filename)
        {
            ...
            // We still haven't found the file yet, the built samples are prefixed with 'csw', so see if that's the case
            if (exeName.ToLower().StartsWith("csw"))
            {
                // Build the new exe name by stripping off the 'csw' prefix and doing the searches again
                string newExeName = exeName.Substring(3, exeName.Length - 3);
    ...
    public class DirectionWidget
    {
        ...
        public static void OnCreateDevice(Device device)
        {
            ...
            effect = Effect.FromFile(device, path, null, null, null, ShaderFlags.NotCloneable, null);
            ...
            mesh = new Mesh(device, path, MeshFlags.Managed, null, null, null);

            // Optimize the mesh for this graphics card's vertex cache 
            // so when rendering the mesh's triangle list the vertices will 
            // cache hit more often so it won't have to re-execute the vertex shader 
            // on those vertices so it will improve perf.     
            Microsoft.DirectX.Generic.GraphicsBuffer<int> adj = new Microsoft.DirectX.Generic.GraphicsBuffer<int>(mesh.NumberFaces * 3);
        ...
        public unsafe void OnRender(ColorValue color, Matrix view, Matrix proj, Vector3 eye)
        {
            ...
            Matrix rotateA = Matrix.LookAtLeftHanded(currentDir, at, up);
            rotateA.Invert();
