...
// File: WDXMUT.cs
//
// DirectX SDK Managed Direct3D for Whidbey sample framework
//
...
namespace Microsoft.Samples.DirectX.UtilityToolkit
{
    ...
    public sealed class Framework : IDisposable
    {
            ...
            int suggestedHeight, IDeviceCreation callback)
        {
            ...
            match.IsWindowed = MatchType.PreserveInput;
            ...
            settings.presentParams.IsWindowed = windowed;
            ...
            {
                Capabilities c = Manager.GetDeviceCapabilities((int)settings.AdapterOrdinal, settings.DeviceType);
        ...
        public void SetDevice(Device device)
        {
            ...
            {
                using (SwapChain swap = backBuffer.GetContainer(Constants.SwapChain) as SwapChain)
                {
            ...
            deviceSettings.AdapterFormat = FindAdapterFormat(deviceSettings.AdapterOrdinal,
                ...
                deviceSettings.presentParams.IsWindowed);
            deviceSettings.BehaviorFlags = creationParams.BehaviorFlags.Value;
        ...
        public void CreateDeviceFromSettings(DeviceSettings deviceSettings, bool preserveInput)
        {
            ...
            if (!preserveInput)
            {
                ...
                match.IsWindowed = MatchType.ClosestToInput;
        ...
        public void ToggleFullscreen()
        {
            ...
            currentSettings.presentParams.IsWindowed = !currentSettings.presentParams.IsWindowed;
            ...
            if (currentSettings.presentParams.IsWindowed)
            ...
            try
            {
                if ((!currentSettings.presentParams.IsWindowed) && (State.IsMaximized))
                {
                    if (WindowForm != null)
                    {
        ...
        private DeviceSettings FindValidDeviceSettings(DeviceSettings settings, MatchOptions match)
        {
            ...
            for (int iAdapter = 0; iAdapter < Enumeration.AdapterInformationList.Count; iAdapter++)
            {
                ...
                for (int iDeviceInfo = 0; iDeviceInfo < adapterInfo.deviceInfoList.Count; iDeviceInfo++)
                {
                    ...
                    {
                        EnumDeviceSettingsCombo deviceSettings = deviceInfo.deviceSettingsList[iDeviceCombo];
        ...
        private DeviceSettings BuildOptimalDeviceSettings(DeviceSettings settings, MatchOptions match)
        {
            ...
            // Windowed
            //---------------------
            if (match.IsWindowed == MatchType.IgnoreInput)
                optimal.presentParams.IsWindowed = true;
            else
                optimal.presentParams.IsWindowed = settings.presentParams.IsWindowed;
            ...
            if (match.AdapterFormat == MatchType.IgnoreInput)
            {
                ...
                if (optimal.presentParams.IsWindowed ||
        ...
        private DeviceSettings BuildValidDeviceSettings(EnumDeviceSettingsCombo deviceCombo, DeviceSettings settings, MatchOptions match)
        {
            ...
            // Windowed 
            //---------------------
            // Just using deviceCombo.IsWindowed
            ...
            else
            {
                if (match.BackBufferCount == MatchType.PreserveInput)
                {
                    bestMultiSampleType = settings.presentParams.MultiSampleType;
                ...
                else if (match.BackBufferCount == MatchType.ClosestToInput)
                {
                    ...
                    for (int i = 0; i < deviceCombo.multiSampleTypeList.Count; i++)
                    {
                        ...
                        if (Math.Abs((int)tempType - (int)settings.presentParams.MultiSampleType) < Math.Abs((int)bestMultiSampleType - (int)settings.presentParams.MultiSampleType))
                        {
            ...
            validSettings.presentParams.MultiSampleType = bestMultiSampleType;
            ...
            validSettings.presentParams.DeviceWindowHandle = deviceCombo.IsWindowed ? State.WindowDeviceWindowed.Handle : State.WindowDeviceFullScreen.Handle;
            validSettings.presentParams.IsWindowed = deviceCombo.IsWindowed;
            ...
            validSettings.presentParams.DisableMultithreadedFlag = true;
        ...
        private bool DoesDeviceComboMatchPreserveOptions(EnumDeviceSettingsCombo deviceCombo, DeviceSettings settings, MatchOptions match)
        {
            ...
            // Windowed
            //---------------------
            if (match.IsWindowed == MatchType.PreserveInput &&
                (deviceCombo.IsWindowed != settings.presentParams.IsWindowed))
                return false;
            ...
            if (match.MultiSample == MatchType.PreserveInput)
            {
                ...
                for (int i = 0; i < deviceCombo.multiSampleTypeList.Count; i++)
                {
                    ...
                    if (msType == settings.presentParams.MultiSampleType &&
        ...
        private float RankDeviceCombo(EnumDeviceSettingsCombo deviceCombo, DeviceSettings settings, DisplayMode adapterMode)
        {
            ...
            // Windowed
            //---------------------
            if (deviceCombo.IsWindowed == settings.presentParams.IsWindowed)
        ...
        private void ChangeDevice(DeviceSettings newDeviceSettings, Device deviceFromApp, bool forceRecreate)
        {
            ...
            if (newDeviceSettings.presentParams.IsWindowed)
            {
                ...
                if (deviceFromApp == null)
                {
                    try
                    {
                        device = new Device((int)newDeviceSettings.AdapterOrdinal, newDeviceSettings.DeviceType,
                            State.WindowFocus.Handle, newDeviceSettings.BehaviorFlags,
            ...
            State.AdapterMonitor = Manager.GetAdapterMonitorHandle((int)newDeviceSettings.AdapterOrdinal);
        ...
        /// </summary>
        private void UpdateDeviceStats(DeviceType deviceType, CreateFlags behaviorFlags, AdapterInformation info)
        {
        ...
        public void Render3DEnvironment()
        {
            ...
            if (State.IsDeviceLost && !State.IsRenderingPaused)
            {
                ...
                if ( ( result = device.CheckCooperativeLevelResult()) != ResultCode.Success )
                {
        ...
        private void PrepareDevice(Device device)
        {
            ...
            if (State.IsShowingCursorWhenFullScreen && !IsWindowed)
            {
                ...
                device.SetCursor(cursor.Handle, false);
        ...
        private void UpdateStaticFrameStats()
        {
            ...
            switch (settings.presentParams.MultiSampleType)
            {
                ...
                case MultiSampleType.NonMaskable:
                    ...
                    break;
                default:
                    multiSampleString = string.Format(" ({0} Multisample)", settings.presentParams.MultiSampleType.ToString());
                    break;
            }
        ...
        private void HandlePossibleSizeChange()
        {
            ...
            if (!State.CurrentDeviceSettings.presentParams.IsWindowed)
        ...
        private uint GetAdapterOridinalFromMonitor(IntPtr monitorHandle)
        {
            ...
            {
                if (Manager.GetAdapterMonitorHandle((int)info.AdapterOrdinal) == monitorHandle)
        ...
        {
            get { return State.CanAutoChangeAdapter; }
        ...
        public bool IsWindowed
        {
            get
            {
                ...
                if ((settings != null) && (settings.presentParams != null))
                    return settings.presentParams.IsWindowed;
                else
                    return false;
            }
        }
        ...
        {
            get { return State.IsNotifiedOnMouseMove; }
        ...
        {
            get { return State.IsOverridingFullScreen; }
