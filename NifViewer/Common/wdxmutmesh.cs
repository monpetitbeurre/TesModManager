...
namespace Microsoft.Samples.DirectX.UtilityToolkit
{
    ...
    public sealed class FrameworkMesh : IDisposable
    {
        ...
        public void SetVertexDeclaration(Device device, VertexElement[] decl)
        {
            ...
            using (systemMemoryMesh)
            {
                using (localMemoryMesh)
                {
                    ...
                    if (systemMemoryMesh != null)
                    {
                        oldDecl = systemMemoryMesh.GetDeclaration();
                        tempSystemMesh = systemMemoryMesh.Clone(device, systemMemoryMesh.Options.Value,
                            decl);
                    }
                    if (localMemoryMesh != null)
        ...
        private void OnResetDevice(object sender, EventArgs e)
        {
            ...
            localMemoryMesh = systemMemoryMesh.Clone(device, (systemMemoryMesh.Options.Value & ~MeshFlags.SystemMemory),
                systemMemoryMesh.VertexFormat);
        }
