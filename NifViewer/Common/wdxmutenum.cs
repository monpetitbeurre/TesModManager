...
namespace Microsoft.Samples.DirectX.UtilityToolkit
{
    ...
    public static class Enumeration
    {
        ...
        // Lists
        private static List<DepthFormat> depthStencilPossibleList = new List<DepthFormat>();
        private static List<MultiSampleType> multiSampleTypeList = new List<MultiSampleType>();
        private static List<PresentInterval> presentIntervalList = new List<PresentInterval>();
        private static List<EnumAdapterInformation> adapterInformationList = new List<EnumAdapterInformation>();
        ...
        public static void Enumerate(IDeviceCreation acceptableCallback)
        {
            ...
            try
            {
                ...
                foreach (AdapterDetails ai in Manager.Adapters)
                {
        ...
        /// </summary>
        private static void EnumerateDevices(EnumAdapterInformation adapterInfo, List<Format> adapterFormatList)
        {
            // Ignore any exceptions while looking for these device types
            DirectXException.IsExceptionIgnored = true;
            ...
            for (uint i = 0; i < deviceTypeArray.Length; i++)
            {
                ...
                deviceInfo.Caps = Manager.GetDeviceCapabilities((int)adapterInfo.AdapterOrdinal, deviceInfo.DeviceType);
            ...
            DirectXException.IsExceptionIgnored = false;
        }
                    ...
                    for (int windowedIndex = 0; windowedIndex < 2; windowedIndex++)
                    {
                        ...
                        if (isPostPixelShaderBlendingRequired)
                        {
                            ...
                            if (!Manager.CheckDeviceFormat((int)adapterInfo.AdapterOrdinal, deviceInfo.DeviceType,
                                    ...
                                    ResourceType.Texture, backbufferFormatsArray[i]))
                                continue; // Unsupported
                        }
        ...
        private static void BuildMultiSampleTypeList(EnumDeviceSettingsCombo deviceCombo)
        {
            foreach (MultiSampleType msType in multiSampleTypeList)
            {
                ...
                if (Manager.CheckDeviceMultiSampleType((int)deviceCombo.AdapterOrdinal,
                    ...
                    deviceCombo.IsWindowed, msType, out quality))
                {
        ...
        private static void BuildPresentIntervalList(EnumDeviceInformation deviceInfo, EnumDeviceSettingsCombo deviceCombo)
        {
            for (int i = 0; i < presentIntervalList.Count; i++)
            {
                ...
                // Note that PresentInterval.Default is zero so you can't do a bitwise
                // check for it, it's always available
                if ((pi == PresentInterval.Default) ||
                    ((deviceInfo.Caps.PresentInterval & pi) != 0))
                {
        ...
        /// </summary>
        public static List<DepthFormat> PossibleDepthStencilFormatList
        {
        ...
        /// </summary>
        public static List<MultiSampleType> PossibleMultisampleTypeList
        {
        ...
        /// </summary>
        public static List<PresentInterval> PossiblePresentIntervalsList
        {
        ...
        /// </summary>
        public static List<EnumAdapterInformation> AdapterInformationList
        {
