using System;
using BSAList=System.Collections.Generic.List<NifViewer.BSAArchive>;
using StringList=System.Collections.Generic.List<string>;
using HashTable=System.Collections.Generic.Dictionary<ulong, NifViewer.BSAArchive.BSAFileInfo>;
using System.IO;
...
namespace NifViewer {
    public class BSAArchive {
        ...
        public static Stream GetTexture(string path) {
            ...
            if(File.Exists(Path.Combine(Program.OblivionPath, path))) return File.OpenRead(Path.Combine(Program.OblivionPath, path));
        ...
        public static NifFile LoadMesh(string path) {
            ...
            switch(ext) {
            case ".nif":
                if(File.Exists(Path.Combine(Program.OblivionPath, path))) return new NifFile(Path.Combine(Program.OblivionPath, path), BasicHLSL.Device);
                ulong hash=GenHash(Path.ChangeExtension(path, null), ext);
        ...
        private static void Load() {
            string path=Program.OblivionPath;
