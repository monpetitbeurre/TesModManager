...
namespace NifViewer {
    public partial class NifFile {
        ...
        public string ParallaxSubset(int i) {
            if(subsets[i].oldColorMap==null) subsets[i].oldColorMap=subsets[i].colorMap;
            try {
                subsets[i].colorMap=HeightMapGen.CreateHeightmapData(subsets[i].oldColorMap, subsets[i].normalMap);
        ...
        public void SaveColorMap(int subset) {
            ...
            string texpath=System.IO.Path.Combine(Program.OblivionPath, subsets[subset].data.path);
