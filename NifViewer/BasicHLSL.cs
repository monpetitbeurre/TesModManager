...
namespace NifViewer {
    public class BasicHLSL : IFrameworkCallback, IDeviceCreation {
        ...
        private const int ToggleParallax = 17;
        ...
        private const int GenHeightmapButton = 23;
        private const int LoadMeshButton = 24;
        private const int SaveMeshButton = 25;
        private const int SaveTexButton = 26;
        private const int ApplyModePicker = 27;
        ...
        public void InitializeApplication() {
            ...
            Checkbox parallaxCheckbox = sampleUi.AddCheckBox(ToggleParallax, "Enable parallax", 35, y+=24, 125, 22, true);
            ...
            y+=19;
            ComboBox applyModePicker = sampleUi.AddComboBox(ApplyModePicker, 35, y+=24, 125, 22);
            applyModePicker.AddItem("Replace", (byte)0);
            applyModePicker.AddItem("Decls", (byte)1);
            applyModePicker.AddItem("Modulate", (byte)2);
            applyModePicker.AddItem("Hilight", (byte)3);
            applyModePicker.AddItem("Parallax", (byte)4);
            applyModePicker.AddItem("obge: Relief", (byte)5);
            applyModePicker.AddItem("obge: Fur", (byte)6);
            Button genHeightButton = sampleUi.AddButton(GenHeightmapButton, "Create height data", 35, y+=32, 125, 22);

            y+=19;
            Button loadButton = sampleUi.AddButton(LoadMeshButton, "Load mesh", 35, y+=24, 125, 22);
            Button saveButton = sampleUi.AddButton(SaveMeshButton, "Save mesh", 35, y+=24, 125, 22);
            Button saveTexButton = sampleUi.AddButton(SaveTexButton, "Save texture", 35, y+=24, 125, 22);
            ...
            parallaxCheckbox.Changed+=new EventHandler(parallaxCheckbox_Changed);
            ...
            genHeightButton.Click+=new EventHandler(genHeightButton_Click);
            loadButton.Click+=new EventHandler(loadButton_Click);
            saveButton.Click+=new EventHandler(saveButton_Click);
            saveTexButton.Click+=new EventHandler(saveTexButton_Click);
            applyModePicker.Changed+=new EventHandler(applyModePicker_Changed);
        ...
        void saveButton_Click(object sender, EventArgs e) {
            if(nifPath==null) return;
            string fullpath=Path.Combine(Program.OblivionPath, nifPath);
            if(!Directory.Exists(Path.GetDirectoryName(fullpath))) Directory.CreateDirectory(Path.GetDirectoryName(fullpath));
            if(nif!=null) nif.SaveNif(fullpath);
        }

        void saveTexButton_Click(object sender, EventArgs e) {
            if(nifPath==null) return;
            if(currentSubset==-1) {
                MessageBox.Show("Can only create height maps for one subset at a time", "Error");
                return;
            }
            if(nif!=null) nif.SaveColorMap(currentSubset);
        }

        void genHeightButton_Click(object sender, EventArgs e) {
            if(currentSubset==-1) {
                MessageBox.Show("Can only create height maps for one subset at a time", "Error");
                return;
            }
            ...
            if(data.nWidth==-1) {
                MessageBox.Show("Can only create height maps for subsets which have a normal map", "Error");
                return;
            }
            if(data.cformat!=Format.Dxt1&&data.cformat!=Format.R8G8B8) {
                MessageBox.Show("Can only create height maps for subsets which don't already have an alpha channel in the colour map", "Error");
                return;
            }
            if(new HeightmapGenForm().ShowDialog()!=System.Windows.Forms.DialogResult.OK) return;
            HeightMapGen.Iterations=HeightmapGenForm.Iterations;
            HeightMapGen.Scale=HeightmapGenForm.Scale;
            string s=nif.ParallaxSubset(currentSubset);
            if(s!=null) MessageBox.Show(s, "Error");
        }

        void subsetControl_Changed(object sender, EventArgs e) {
            ComboBox cb=(ComboBox)sender;
            ComboBox cmbApply=sampleUi.GetComboBox(ApplyModePicker);
            if(cb.NumberItems>0) currentSubset=(int)cb.GetSelectedData();

            if(currentSubset==-1) {
                subsetDesc=null;
                cmbApply.SetSelected(0);
            } else {
                NifFile.SubsetData data=nif.GetSubsetData(currentSubset);
                ...
                cmbApply.SetSelected(nif.GetApplyMode(currentSubset));
            }
        }
        ...
        public static int Run(int adapter, int aa, int af, string mesh) {
            ...
            using(Framework sampleFramework = new Framework()) {
                ...
                try {
                    ...
                    sampleFramework.CreateWindow("NIF viewer");
                    ...
                    sampleFramework.CreateDevice(adapter, true, 800, 600, null); //sample);
