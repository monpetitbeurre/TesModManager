...
namespace ESPMHider
{
	...
	internal sealed class Program
	{
		...
		{
			{
				...
				if (dataDir.Exists)
				{
					...
					{
						espm.MoveTo(Path.Combine(Program.DataFolderName, espm.Name));
					}
				}
			}
			
			{
				...
				if (dataDir.Exists)
				{
					...
					{
                        try { fi.MoveTo(Path.Combine(Program.DataFolderName, fi.Name.Substring(0, fi.Name.Length - 6))); }
		...
		public static void HideESPM()
		{
			...
			{
				StreamReader sr = new StreamReader(Path.Combine(OblivionESPDir, "plugins.txt"));
