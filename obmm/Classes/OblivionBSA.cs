...
namespace OblivionModManager.Classes {
    class BSAArchive {
        ...
        public static bool CheckForTexture(string path) {
            ...
            if(File.Exists(Path.Combine(Program.currentGame.DataFolderPath,path))) return true;
        ...
        public static byte[] GetMesh(string path) {
            ...
            string ext=Path.GetExtension(path);
            if(ext==".nif") {
                if(File.Exists(Path.Combine(Program.currentGame.DataFolderPath,path))) return File.ReadAllBytes(Path.Combine(Program.currentGame.DataFolderPath,path));
                ulong hash=GenHash(Path.ChangeExtension(path, null), ext);
        ...
        private static void Load(bool populateAll) {
            foreach(string s in Directory.GetFiles(Program.currentGame.DataFolderPath, "*.bsa")) new BSAArchive(s, populateAll);
            Loaded=true;
        }
